        ...
        if (receive_packet.data[0] == 'd') {
            ...
            while (file_size > 0) {
                ...
                for(int i=0;i<cwnd;i++){
                    ...
                    mt19937 gen(rd());  //Mersenne Twister 19937隨機數引擎
                    uniform_real_distribution<> dis(0, 1);
                    double loss_rate = 1e-1;  // Mean packet loss rate
                    ...
                    recv(new_fd, &receive_packet, sizeof(receive_packet), 0);
                    cout << "       Receive an ACK packet (seq_num = " << send_packet.h.seq_num << ", ack_num = " << receive_packet.h.seq_num+1 << ")" << endl;
                    if(rwnd <= 0){
                        rwnd = 512*1024;
                    }
                }
