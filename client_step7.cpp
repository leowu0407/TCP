...
int main()
{
    ...
    while (1) {
        ...
        //get video
            ...
            s = send_packet.data+4;
            int recv_seq = receive_packet.h.seq_num;
            cout << "receive video: " << s << " from " << ip_address << " : " << htons(server.sin_port) << endl;
            while(1){
                recv(sock_fd, &receive_packet, sizeof(receive_packet), 0);
                if((recv_seq+1) != receive_packet.h.seq_num){
                    ;
                }
                else{
                    send_packet.h.ack_num = receive_packet.h.seq_num+1;
                    recv_seq = receive_packet.h.seq_num;
                    send_packet.h.seq_num += 1024;
                }
